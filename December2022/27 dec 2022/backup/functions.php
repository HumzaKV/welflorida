...
function portfolio_page_template($template)
{
    ...
    if( $post && $post->post_type == 'memberpressproduct' ) {
        $template = locate_template(array('portfolio-page-template.php'));
    }
...
function cf_custom_template_include($template) {
    ...
    // else if( is_singular( 'memberpressproduct' ) ) {
    else if( $post && $post->post_type == 'memberpressproduct' ) {
...
function kv_mepr_transaction_activate($txn) {
    // $id = sanitize_key($txn->id);
    $value = 'complete';
...
add_action('mepr-txn-status-complete', 'kv_mepr_transaction_activate');

// add_action('wp_head', 'kv_abcd');
// function kv_abcd() {
//     if( @$_GET['asdf'] == 'go' ) {
//         $tdata = MeprTransaction::get_one('488', ARRAY_A);
//         $txn = new MeprTransaction();
//         $txn->load_data($tdata);
//         kv_mepr_transaction_activate($txn);
//     }
// }
